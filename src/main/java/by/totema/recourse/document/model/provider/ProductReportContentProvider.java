...
public class ProductReportContentProvider implements ContentProvider<Object, OrderReportDto> {
    @Override
    ...
    public List<String> getHeaders() {
        return Arrays.asList("Product", "Quantity", "Total Price", "Total Cost", "Total Gross Margin", "Currency");
    }
    ...
    public List<List<String>> createRows(Collection<OrderReportDto> orderReportDtos) {
        return orderReportDtos.stream()
                .map(dto -> Arrays.asList(dto.getProduct(), dto.getQuantity().toString(), dto.getTotalPrice().toString(), dto.getTotalCost().toString(), dto.getTotalGrossMargin().toString(), dto.getCurrency()))
