...
public class OfficeReportContentProvider implements ContentProvider<Object, OrderReportDto> {
    @Override
    ...
    public List<String> getHeaders() {
        return Arrays.asList("Office", "Country", "Quantity", "Total Price", "Total Cost", "Total Gross Margin", "Currency");
    }
    ...
    public List<List<String>> createRows(Collection<OrderReportDto> orderReportDtos) {
        return orderReportDtos.stream()
                .map(dto -> Arrays.asList(dto.getOffice(), dto.getCountry(), dto.getQuantity().toString(), dto.getTotalPrice().toString(), dto.getTotalCost().toString(), dto.getTotalGrossMargin().toString(), dto.getCurrency()))
